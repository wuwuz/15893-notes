...
\begin{theorem}[Time-space tradeoff for preprocessing PIR~\cite{CK20}]
    ...
    Then,  
    $(S+1)(T+1) \ge N$. \ignore{TODO: edit the theorem based on what we can prove later}
\label{thm:lb}
\end{theorem}
...
We require the protocol to have {\it perfect correctness},
...
In particular, it is perfectly okay for the query 
...
\section{Time-Space Tradeoff for Preprocessing PIR}
We will next prove a time-space tradeoff for preprocessing PIR.
...
Yao's box problem with probablistic correctness $\delta(n)$,  
iff 
for any $\DB\in \{0, 1\}^n$, 
it holds that 
\[
...
Given a possibly randomized protocol
that solves Yao's box problem with correctness probability 
0.8 or higher, 
let $S$ be the client space and 
let $T$ be the server time during the query.
It must be that $S \cdot T \geq \Omega(n)$.
\label{thm:yao-prob}
\end{theorem}
...
\begin{fact}
Let $\epsilon \in (0, 1/2)$ be some constant. 
Given a set of $M$ items, 
pick a subset of size at most $(1/2 - \epsilon)M$. 
The subset 
can be encoded using $M \cdot (1- 2.86 \epsilon^2)$ bits. 
\label{fct:encodeset}
\end{fact}
\begin{proof}
The number of ways to choose a subset of size
at most $(1/2 - \epsilon)M$
is 
\[
\sum_{i = 0}^{(1/2 - \epsilon)M}
{M \choose i}
\leq 2^{\mathbb{H}(1/2-\epsilon) M}
\]
where $\mathbb{H}$ is the binary entropy function, and the inequality 
is due to Theorem 3.1 of Galvin~\cite{galvin} --- you can also try 
proving it as exercise yourself!
Taking $\log_2$ of the above, we get that 
$\log_2(2^{\mathbb{H}(1/2-\epsilon) M}) = 
\mathbb{H}(1/2-\epsilon)\cdot M \leq M \cdot (1-2.86\epsilon^2)$
where the last inequality is by Taylor expansion. 

\ignore{
its not a major thing at all- but there is a small mistake in the proof of fact 2- the sterling step seems to use M! \le (M/e)^n, the inequality actual goes the other way (and hence using it in the denominator is fine)

You can directly use the following inequality for simplicity- it ends up in the same expression you get before the taylor expansion:
for \alpha \in(0,0.5), \sum_{i=0}^{\alpha*n} {n \choose i} \leq 2^{H(\alpha)} where H is the binary entropy function- 
(an interested student can try to do the proof of this step as an exercise)
you can add this note as a clean proof of the reference of this inequality in case you use it https://arxiv.org/pdf/1406.7872.pdf
...
of size at most $(1/2 - \epsilon)M$
among $M$ 
elements is at most $(1/2 - \epsilon)M \cdot f + 1$.

Taking $\log_2 $ of  
$(1/2 - \epsilon)M \cdot f + 1$, 
and then using Taylor expansion, we get that 
for sufficiently large $M$, 
$\log_2((1/2 - \epsilon)M \cdot f + 1) \leq 
\log_2\left(
\left(\frac{1}{\frac14- \epsilon^2}\right)^{(\frac12 - \epsilon)M}
\cdot
\left(\frac{1}{\frac12 + \epsilon}\right)^{2\epsilon M}
\right)  + 
\log_2(M)
\leq M \cdot (1 - 2.86\epsilon^2)$.

\ignore{TO FIX here: 
the set can also be smaller than this size, consider all possible sizes.} 
}
\end{proof}
...
$\E[|R|] = n/10T$. Further, 
by Chernoff bound, 
$\Pr[|R| \leq 0.09n/T] \geq \exp(-\Omega(n/T))$, 
and $\Pr[|R| \geq 0.11n/T] \geq \exp(-\Omega(n/T))$.
\label{fct:Rsize}
\end{fact}

\begin{claim}
%$\E[|G|] \geq 0.09/T$. 
$\Pr[|G| \geq 0.026n/T] \geq 0.74$ for sufficiently large $n$.
...
with probability at least $1 - \exp(-\Omega(n/T)) - 1/4$, 
$|G| \geq 0.09n/T - 0.064n/T = 0.026n/T$.
\end{proof}

\begin{claim}
For sufficiently large $n$, with probability at least $0.3$, %\elaine{FILL}, 
...
\]
Therefore,  
it must be that with probability $p \geq 0.3$, 
$|G_0| - |G_1| \geq 0.01\cdot n/T$, since otherwise, 
\[
\E[|G_0| - |G_1|]
\leq 
(p - \exp(-\Omega(n/T))) \cdot 0.11 n/T
 + (1-p) \cdot 0.01 n/T
...
%\elaine{FIX}
$1-(1-0.74)- (1-0.3) = 0.04$ over the choice of $R$, 
...
\paragraph{Completing the proof of \Cref{thm:yao-prob}.}
Suppose we have a (possibly randomized)
protocol $\Pi$ for solving Yao's box problem, and for any $\DB$,
it achieves  
probabilistic correctness $0.8$ for a random query index.
Then, it must be that we can find 
some good ${\sf coins}$, such that 
for at least $0.1$ fraction of the databases,
$\Pi^{\sf coin}$ 
gives a correct answer on at least $3/4$ of the indices.
Since otherwise, 
$\Pi$ can achieve  
correctness probability at most $0.1 \cdot 1 + 0.9 \cdot 3/4 < 0.775$ 
for a random $\DB$ and random index $i \in [n]$ which
violates the assumption that $\Pi$ has $0.8$ probabilistic correctness.

Now, fixing ${\sf coins}$ to be the good choice
as mentioned above, we can use the above encoding/decoding procedure  
to encode the database. 
We say that a $\DB$ is good if 
$\Pi^{\sf coin}$ 
is correct on at least $3/4$ of the indices for $\DB$.
Henceforth, without loss of generality, we may
assume that $T \in [2, o(n)]$.
By \Cref{lem:prob-yao-key-lemma}, 
as long as $\DB$ is good, using the above encoding/decoding procedure, 
decoding 
is correct with probability at least $0.04$.
Further, we know that $0.1$ fraction of the $\DB$s are good.
Therefore, when we randomly choose $\DB$, the probability of correct
decoding is at least $0.1 \cdot 0.04 = \Theta(1)$.
By \Cref{fct:shannonprob}, the length of the encoding 
is at least $n - \Theta(1)$.
Therefore, we have that 
\[
...
%\[
%\Pr\left[\DB \getr \{0, 1\}^n, i \getr [n]: \ 
%\Pi^{\sf coins} \text{ is correct for $(\DB, i)$} \right]
%\geq 3/4
%\]
