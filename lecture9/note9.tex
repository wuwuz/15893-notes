...
\noindent where deeper colors represent larger values, and the arrows point to the larger values. Note that in the top $k-n/2$ rows (until the dashed line), the value on the right column must be greater than or equal to that on the left column.
