...
We start by introducing the Bucket Oblivious Sorting algorithm~\cite{bucket} that has an $\mathcal{O}(n\log n (\log\log n)^2)$ complexity. Afterward, we will discuss the improvement to speed it up to $\mathcal{O}(n\log n)$. In general, the framework of BucketSort is as follows: 
