...
\newcommand{\Coutput}{\ensuremath{{\sf Coutput}}}
...
In recent lectures, the construction of two-server Private Information Retrieval (PIR) schemes with sublinear bandwidth was discussed. The current focus is on the single-server setting, questioning the feasibility of achieving sublinear bandwidth. It was established that under cryptographic assumptions, such as fully homomorphic encryption (FHE), one can indeed achieve essentially constant bandwidth. 
We now see two lower bounds for single-server PIR. 

\begin{enumerate}
    \item \textit{A perfect (or even statistical) PIR scheme without cryptographic assumptions require linear bandwidth;}

    \item \textit{A single-server PIR scheme with non-trivial bandwidth implies Oblivious Transfer.}
...
The high-level idea of the first lower bound is to show that there exists a computationally unbounded extractor, such that given any database $\DB$ and any query $q$, the extractor can extract the whole database given the communication script.
This shows that the communication script has at least $n$-bit of entropy, and thus it is at least $n$-bit long. 
...
The following theorem has been a folklore lower bound and is formalized by Damgård, Larsen and Nielsen~\cite{DLN19}.

\begin{theorem}[\cite{DLN19}]
1-server PIR scheme with perfect (or statistical) correctness and perfect (or statistical) privacy must have $\Omega(n)$ bandwidth where $n = |\DB|$, which even holds for multi-round protocols. 
\end{theorem}
...
\paragraph{Notations.} Given any database $\DB$ of size $n$, any client query $i\in[n]$,    
...
We want to show that this protocol must have at least $n$ bandwidth and the way we are going to propose an encoding scheme. We are going to see that by perfect privacy and perfect correctness of the PIR scheme, given any possible communication script $C$, a computationally unbounded extractor can extract all original database bits. Then, the communication script $C$ can actually be seen as the codeword for the database, and the decoder just runs the extractor to reconstruct the database. It gives a uniquely decodable scheme and by Shannon's source coding theorem, the expected length of the codeword has be be at least $n$-bit.

We now define the encoding scheme and the decoding scheme. Notice that the encoder and the decoder algorithms do not need to be efficient (polynomially bounded).

\begin{itemize}
    \item \textit{Encoding}. Given any $n$-bit database $\DB$, the encoding for $\DB$ is the shortest possible communication script $C$ that is compatible with the $\DB$. That is, there exist $r_1,r_2\in \{0,1\}^*, i\in[n]$ such that $\langle \Server(\DB, r_1) \leftrightarrow \Client(i, r_2) \rangle$.

    \item \textit{Decoding}. Given any code word $C$, the decoder algorithm will reconstruct the $i$-th bit of $\DB$ as follows. The algorithm fixes $i$ and enumerates $r_2$ until $C$ is a possible communication script given the client's inputs are $i,r_2$. Then, the $i$-th bit of $\DB$ is reconstructed by $\Coutput\left(C, i, r_2\right)$.
\end{itemize}
...
Based on the claims, we show that this encoding-decoding scheme is a uniquely decodable code, i.e., $\forall~\DB\in \{0,1\}^n$, $\Pr[\textsf{Decode}(\textsf{Encode(DB)}]=1$. 
Then, by Shannon's source coding theorem, we have that 
\[
    \E_{\DB\getr\{0,1\}^n}\left[\textsf{Encode}(\DB)\right]\ge H(\DB) = n.
\]

Here, $H(\DB)$ is the entropy of $\DB$.
...
\subsection*{Concavity of Entropy}
...
Let $X$ and $Y$ be random variables with $X \in D_X$ and $Y \in D_Y$. Let $\hat{X} = f(Y)$ and $p = P(X \neq \hat{X})$.
\begin{align*}
