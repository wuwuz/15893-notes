...
\begin{lemma}
    Suppose $X_1,\dots,X_n$ are binary random variables such that for 
each $i$, $\Pr[X_i = 1] = \frac{1}{2}+\delta_i$ where  $\delta_i \in( -\frac{1}{2}, \frac{1}{2})$.
    Then, we have that 
    \[
    \Pr[X_1 \oplus... \oplus X_n = 1] = \frac{1}{2}+ \prod_{i = 1}^n (2\delta_i)
%\delta(2 \delta)^{n-1}.
    \]
\end{lemma}
...
Suppose the statement holds for $\Pr[X_1 \oplus \ldots \oplus X_m = 1]$, we want
to show that it holds for $\Pr[X_1 \oplus \ldots \oplus X_{m+1}= 1]$.
Write $\Pr[X_1 \oplus \ldots \oplus X_i = 1] = \frac12 + p_m$.
By a simple conditional probability calculation, 
\[
\Pr[X_1 \oplus \ldots \oplus X_{m+1} = 1] = \left(\frac12+ p_m\right) 
\cdot \left(\frac12 + \delta_{m+1}\right) + 
\left(\frac12-p_m\right) \cdot \left(\frac12 - \delta_{m+1}\right)
\]
Therefore, 
$\Pr[X_1 \oplus \ldots \oplus X_{m+1} = 1] = 
\frac12 + 2p_m \delta_{m+1}$ which gives the lemma.
\ignore{
\text{Given:}
\[
